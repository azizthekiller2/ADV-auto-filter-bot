...
class Database:    
    ...
    async def delete_all_msg(self):
        ...
        logging.info("All filenames notification have been deleted.")
        return True
    ...
    async def add_user(self, id, name):
        # Use upsert to avoid DuplicateKeyError on concurrent calls
    ...
    async def ban_user(self, user_id, ban_reason="No Reason"):
        ...
        await self.col.update_one(
            {'id': user_id},
            {'$set': {'ban_status': ban_status, 'id': user_id}},
            upsert=True
        )
    ...
    async def add_chat(self, chat, title):
        ...
        await self.grp.update_one(
            {'id': int(chat)},
            {'$setOnInsert': self.new_group(chat, title)},
            upsert=True
        )
    ...
    async def dreamx_reset_settings(self):
        try:
            ...
            logging.error(f"[ERROR] Failed to reset group settings: {e}")
            raise  
    ...
    async def get_notcopy_user(self, user_id):
        ...
        if not user:
            ...
            return res  # return the dict, not InsertOneResult
        return user
    ...
    async def has_premium_access(self, user_id):
        ...
        if user_data:
            ...
            if expiry_time is None:
                return False
            elif isinstance(expiry_time, datetime.datetime) and datetime.datetime.now(pytz.utc) <= expiry_time.replace(tzinfo=pytz.utc):
                return True
            else:
    ...
    async def give_free_trial(self, user_id):
        user_id = user_id
        seconds = 5*60         
        expiry_time = datetime.datetime.utcnow() + datetime.timedelta(seconds=seconds)
    ...
    async def connect_group(self, group_id, user_id):
        # Atomic upsert with $addToSet eliminates the read-then-write race condition
    ...
    async def get_db_size(self):
        try:
            ...
            logging.error(f'get_db_size error: {e}')
            return 0
