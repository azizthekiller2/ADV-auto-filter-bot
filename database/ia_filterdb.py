...
async def check_db_size(db):
    try:
        ...
        logger.warning(f"Error Checking Database Size: {e}")
        return 0
    ...
    if MULTIPLE_DB:
        try:
            ...
            if (c1 if isinstance(c1, int) else 0) + (c2 if isinstance(c2, int) else 0) > 0:
                logger.info(f"[SKIP] '{file_name}' already in Primary or Secondary DB.")
                return False, 0
...
async def get_search_results(chat_id, query, file_type=None, max_results=None, offset=0, filter=False):
    ...
    # Helper: run a DB2 coroutine with a hard 5-second timeout so a bad/slow
    # DB2 connection never blocks the primary DB results.
    async def _safe_db2(coro, default):
        try:
    ...
    if ULTRA_FAST_MODE:
        limit = max_results + 1
        primary_coro = Media.find(filter_mongo).sort("$natural", -1).skip(offset).limit(limit).to_list(length=limit)
        if MULTIPLE_DB:
            db2_coro = _safe_db2(
                Media2.find(filter_mongo).sort("$natural", -1).skip(offset).limit(limit).to_list(length=limit),
                default=[]
            )
            results = await asyncio.gather(primary_coro, db2_coro, return_exceptions=True)
        else:
            results = await asyncio.gather(primary_coro, return_exceptions=True)

        files = results[0] if not isinstance(results[0], BaseException) else []
        if MULTIPLE_DB and len(results) > 1 and not isinstance(results[1], BaseException):
        ...
        primary_count_coro = Media.count_documents(filter_mongo)
        primary_find_coro  = Media.find(filter_mongo).sort("$natural", -1).skip(offset).limit(max_results).to_list(length=max_results)

        if MULTIPLE_DB:
            db2_count_coro = _safe_db2(Media2.count_documents(filter_mongo), default=0)
            db2_find_coro  = _safe_db2(
                Media2.find(filter_mongo).sort("$natural", -1).skip(offset).limit(max_results).to_list(length=max_results),
                default=[]
            )
            count_results, find_results = await asyncio.gather(
                asyncio.gather(primary_count_coro, db2_count_coro, return_exceptions=True),
                asyncio.gather(primary_find_coro,  db2_find_coro,  return_exceptions=True)
            )
        else:
            count_results, find_results = await asyncio.gather(
                asyncio.gather(primary_count_coro, return_exceptions=True),
                asyncio.gather(primary_find_coro,  return_exceptions=True)
            )

        total_results = sum(r for r in count_results if not isinstance(r, BaseException))
        files = find_results[0] if not isinstance(find_results[0], BaseException) else []
        if MULTIPLE_DB and len(find_results) > 1 and not isinstance(find_results[1], BaseException):
