...
def _safe_int_env(key, default='-100'):
    """Extract integer from env var, handling 'ID: -123' style values."""
    import re as _re
...
_api_id_raw = environ.get('API_ID', '').strip()
if not _api_id_raw:
    raise SystemExit("\n\u274c FATAL: API_ID is not set! In Railway go to Variables tab and add API_ID (from my.telegram.org)\n")
API_ID = int(_api_id_raw) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', '').strip()  # API Hash from my.telegram.org
if not API_HASH:
    raise SystemExit("\n\u274c FATAL: API_HASH is not set! In Railway go to Variables tab and add API_HASH (from my.telegram.org)\n")
BOT_TOKEN = environ.get('BOT_TOKEN', '').strip()    # Bot token from @BotFather
if not BOT_TOKEN:
    raise SystemExit("\n\u274c FATAL: BOT_TOKEN is not set! In Railway go to Variables tab and add BOT_TOKEN (from @BotFather)\n")
...
PICS = PICS or ['https://graph.org/file/56b5deb73f3b132e2bb73.jpg']  # Guard: never allow empty PICS list
...
LOG_CHANNEL = _safe_int_env('LOG_CHANNEL', -100)  # Log channel id (make sure bot is admin)
BIN_CHANNEL = _safe_int_env('BIN_CHANNEL', -100)  # Bin channel id (make sure bot is admin)
PREMIUM_LOGS = _safe_int_env('PREMIUM_LOGS', -100)  # Premium logs channel id
...
VERIFY_IMG = environ.get("VERIFY_IMG", "https://raw.githubusercontent.com/azizthekiller123/Auto-filter-bot-4/main/assets/verified.png")

TUTORIAL = environ.get("TUTORIAL", "https://t.me/jrzitxtx/56")   # Tutorial link for verification
TUTORIAL_2 = environ.get("TUTORIAL_2", "https://t.me/BackupChannel5211")   # Second tutorial link for verification
TUTORIAL_3 = environ.get("TUTORIAL_3", "https://t.me/Moviebot123")   # Third tutorial link for verification

# Verification (Must Fill All Veriables. Else You Got Error
SHORTENER_API = environ.get("SHORTENER_API", "e1edb482d86a155fd50fc25e4a45a62cb1afbc58") # Shortener API key
SHORTENER_WEBSITE = environ.get("SHORTENER_WEBSITE", "teraboxlinks.com") # Shortener website (domain only, no https:// prefix needed)

SHORTENER_API2 = environ.get("SHORTENER_API2", "")  # Shortener API key for second website
SHORTENER_WEBSITE2 = environ.get("SHORTENER_WEBSITE2", "")  # Shortener website for second website (leave blank if not using)

SHORTENER_API3 = environ.get("SHORTENER_API3", "")  
SHORTENER_WEBSITE3 = environ.get("SHORTENER_WEBSITE3", "")  # Shortener website for third website (leave blank if not using)

TWO_VERIFY_GAP = int(environ.get('TWO_VERIFY_GAP', "0"))   # Time gap before 2nd shortener fires (0 = immediate chaining)
THREE_VERIFY_GAP = int(environ.get('THREE_VERIFY_GAP', "0"))  # Time gap before 3rd shortener fires (0 = immediate chaining)    

# ============================
# Channel & Group Links Configuration
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/Moviebot123') # Group link for the bot
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/Moviebot123') # Owner link for the bot
UPDATE_CHNL_LNK = environ.get('UPDATE_CHNL_LNK', 'https://t.me/backupchannek') # Update channel link for the bot
...
try:
    DELETE_TIME = max(30, int(environ.get("DELETE_TIME", "300")))  # deletion time in seconds (min 30s, default 5 min)
except ValueError:
    DELETE_TIME = 300
...
IMDB = is_enabled(environ.get('IMDB', "True" if environ.get('TMDB_API_KEY') else "False"), bool(environ.get('TMDB_API_KEY')))    # IMDB Results On (True) / Off (False)
TMDB_ON_SEARCH = is_enabled(environ.get('TMDB_ON_SEARCH', "True" if environ.get('TMDB_API_KEY') else "False"), bool(environ.get('TMDB_API_KEY')))    # Use TMDB Poster On Search Results
...
NO_PORT = is_enabled(environ.get('NO_PORT', 'True'), True)  # Railway/Heroku expose port 443, not the internal port
APP_NAME = None
if 'DYNO' in environ:
    ON_HEROKU = True
...
FQDN = str(getenv('FQDN') or getenv('RAILWAY_PUBLIC_DOMAIN') or BIND_ADRESS) if not ON_HEROKU or getenv('FQDN') else APP_NAME+'.herokuapp.com'
URL = "https://{}/".format(FQDN) if NO_PORT else "https://{}:{}/".format(FQDN, PORT) if ON_HEROKU or NO_PORT else "https://{}:{}/".format(FQDN, PORT)
SLEEP_THRESHOLD = int(environ.get('SLEEP_THRESHOLD', '60'))
WORKERS = int(environ.get('WORKERS', '4'))
SESSION_NAME = str(environ.get('SESSION_NAME', 'MovieBot'))
MULTI_CLIENT = False
name = str(environ.get('name', 'MOVIEBOT123'))
PING_INTERVAL = int(environ.get("PING_INTERVAL", "1200"))  # 20 minutes
TIMEZONE = environ.get("TIMEZONE", "Asia/Kolkata")  # Timezone for scheduled tasks
if 'DYNO' in environ:
    ON_HEROKU = True
    APP_NAME = str(getenv('APP_NAME'))
else:
    ON_HEROKU = False
HAS_SSL = is_enabled(getenv('HAS_SSL', 'True'), True)  # Already correct
if HAS_SSL:
    URL = "https://{}/".format(FQDN) if NO_PORT else "https://{}:{}/".format(FQDN, PORT)
else:
    URL = "http://{}/".format(FQDN) if NO_PORT else "http://{}:{}/".format(FQDN, PORT)
