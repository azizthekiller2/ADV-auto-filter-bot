...
from plugins.botutils.Imdbposter import get_movie_detailsx
...
async def search_gagala(text):
    ...
    async with aiohttp.ClientSession() as _sess:
        async with _sess.get(url, headers=usr_agent) as _resp:
...
async def raw_shorten(api_key: str, site: str, long_url: str) -> str:
    """
    Fallback shortener that sends a plain GET without format=json.
    Works for sites like teraboxlinks.com whose API rejects that param.
    Expects JSON response with a 'shortenedUrl', 'shortened_url',
    'short_url', or 'shortlink' key.
    """
    from urllib.parse import quote as _quote
    base = site if site.startswith("http") else f"https://{site}"
    req_url = f"{base}/api?api={api_key}&url={_quote(long_url, safe='')}"
    async with aiohttp.ClientSession() as _s:
        async with _s.get(req_url, timeout=aiohttp.ClientTimeout(total=10)) as _r:
            if _r.status != 200:
                raise Exception(f"HTTP {_r.status} from {site}")
            data = await _r.json(content_type=None)
    if isinstance(data, dict):
        short = (data.get("shortenedUrl") or data.get("shortened_url")
                 or data.get("short_url") or data.get("shortlink"))
        if short and str(short).startswith("http"):
            return str(short)
        if data.get("status") == "error":
            raise Exception(data.get("message") or str(data))
    raise Exception(f"Unrecognised API response from {site}: {str(data)[:200]}")
...
async def get_shortlink(link, grp_id, is_second_shortener=False, is_third_shortener=False):
    ...
    # Reject placeholder or invalid domain names (e.g. shortner_website_url)
    # A valid domain has no underscores and contains at least one dot.
    # If 2nd/3rd shortener is not configured, fall back to 1st shortener.
    _c = (site or '').split('/')[0].lower()
    if not site or '_' in _c or '.' not in _c:
        if is_second_shortener or is_third_shortener:
            api, site = settings['api'], settings['shortner']
            _c = (site or '').split('/')[0].lower()
        if not site or '_' in _c or '.' not in _c:
            raise ValueError(
                f"Shortener site {site!r} is not a valid domain. "
                "Run /remove_shortner to reset, then /set_shortner with a real domain."
            )
    shortzy = Shortzy(api, site)
    try:
        link = await shortzy.convert(link)
    except Exception as _first_err:
        logging.warning(f"get_shortlink convert failed ({site}): {_first_err} — trying get_quick_link")
        try:
            link = await shortzy.get_quick_link(link)
        except Exception as _second_err:
            logging.warning(f"get_shortlink get_quick_link failed ({site}): {_second_err} — trying raw request")
            try:
                link = await raw_shorten(api, site, link)
            except Exception as _third_err:
                raise Exception(f"Shortener [{site}] failed: {_third_err}") from _third_err
    if not link or not str(link).startswith(("http://", "https://")):
        raise Exception(f"Shortener [{site}] returned an invalid URL: {link!r}. Use /set_shortner to update or /remove_shortner to reset.")
    # Detect when Shortzy silently returns the raw API call URL instead of a short link.
    # That URL contains '?api=' with the key embedded — never expose it to users.
    if "?api=" in str(link):
        raise Exception(f"Shortener [{site}] returned its own API call URL instead of a short link — API key may be invalid or rate-limited.")
    return link
...
def clean_filename(file_name):
    import re as _re
    # Owner channels — keep these in filenames, strip all others
    OWNER_CHANNELS = {'backupchannel5211', 'backupchannek'}
    unwanted = {word.lower() for word in BAD_WORDS}

    def _is_owner(name):
        return name.lower() in OWNER_CHANNELS

    # Strip file extension so it doesn't bleed into channel name matching
    file_name = _re.sub(
        r'\.(mkv|mp4|avi|mov|wmv|flv|webm|m4v|srt|ass|sub|txt)$',
        '', file_name, flags=_re.IGNORECASE
    )

    # Handle -@channel or .@channel glued watermarks (keep if owner, remove if not)
    def _strip_glued(m):
        sep, channel = m.group(1), m.group(2)
        return (sep + '@' + channel) if _is_owner(channel) else ''
    file_name = _re.sub(r'([\-\.])@([A-Za-z0-9]+)', _strip_glued, file_name)

    # Handle @OtherChannel_ActualContent — strip channel prefix, keep content
    def _strip_at_prefix(m):
        channel, rest = m.group(1), m.group(2)
        return ('@' + channel + '_' + rest) if _is_owner(channel) else rest
    file_name = _re.sub(r'@([A-Za-z0-9]+)_([A-Za-z0-9]\S*)', _strip_at_prefix, file_name)

    # Rebuild word list — keep owner @tags, drop all other @tags, drop bad words
    words = []
    for word in file_name.split():
        if word.startswith('@'):
            # check both full name and part before _ e.g. @backupchannek_Movie
            channel_part = word[1:].split('_')[0]
            if _is_owner(word[1:]) or _is_owner(channel_part):
                words.append(word)
        elif word.startswith('[') or word.startswith('www.'):
            pass
        elif word.lower() in unwanted:
            pass
        else:
            words.append(word)
