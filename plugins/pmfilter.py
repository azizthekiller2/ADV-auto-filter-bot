...
lock = asyncio.Lock()
_req_cooldown: dict = {}   # user_id -> last request ts (rate-limit)

logger = logging.getLogger(__name__)
logger.setLevel(logging.ERROR)

# tracemalloc: disabled in production (uncomment locally for memory profiling)
# tracemalloc.start()
...
_MAX_DICT = 2000   # max entries per session-cache dict before pruning

def _prune(d: dict, limit: int = _MAX_DICT) -> None:
    """Remove the oldest half of dict entries when it exceeds limit."""
    if len(d) > limit:
        excess = len(d) - limit // 2
        for k in list(d.keys())[:excess]:
            d.pop(k, None)
...
async def give_filter(client, message):
    ...
    if message.chat.id != SUPPORT_CHAT_ID:
        if message.from_user:
            _ban = await db.get_ban_status(message.from_user.id)
            if _ban.get("is_banned"):
                return
        ...
        if total_results == 0:
            return
        _mention = message.from_user.mention if message.from_user else "ᴛʜᴇʀᴇ"
...
async def pm_text(bot, message):
    ...
    _ban_st = await db.get_ban_status(user_id)
    if _ban_st.get("is_banned"):
        return await message.reply_text(
            f"🚫 You are banned from using this bot.\nReason: {_ban_st.get('ban_reason', 'N/A')}"
        )
    if EMOJI_MODE:
        try:
            await message.react(emoji=random.choice(REACTIONS), big=True)
    ...
    try:
        ...
        if pm_search:
            ...
            await message.reply_text(
                text=(
                    f"<b>🙋 ʜᴇʏ {user} 😍 ,\n\n"
                    "𝒀𝒐𝒖 𝒄𝒂𝒏 𝒔𝒆𝒂𝒓𝒄��� 𝒇𝒐𝒓 𝒎𝒐𝒗𝒊𝒆𝒔 𝒐𝒏𝒍𝒚 𝒐𝒏 𝒐𝒖𝒓 𝑴𝒐𝒗𝒊𝒆 𝑮𝒓𝒐𝒖𝒑. 𝒀𝒐𝒖 𝒂𝒓𝒆 𝒏𝒐𝒕 𝒂𝒍𝒍𝒐𝒘𝒆𝒅 𝒕𝒐 𝒔𝒆𝒂𝒓𝒄𝒉 𝒇𝒐𝒓 𝒎𝒐𝒗𝒊𝒆𝒔 𝒐𝒏 𝑫𝒊𝒓𝒆𝒄𝒕 𝑩𝒐𝒕. 𝑷𝒍𝒆𝒂𝒔𝒆 𝒋𝒐𝒊𝒏 𝒐𝒖𝒓 𝒎𝒐𝒗𝒊𝒆 𝒈𝒓𝒐𝒖𝒑 𝒃𝒚 𝒄𝒍𝒊𝒄𝒌𝒊𝒏𝒈 𝒐𝒏 𝒕𝒉𝒆  𝑹𝑬𝑸𝑼𝑬𝑺𝑻 𝑯𝑬𝑹𝑬 𝒃𝒖𝒕𝒕𝒐𝒏 𝒈𝒊𝒗𝒆𝒏 𝒃𝒆𝒍𝒐𝒘 𝒂𝒏𝒅 𝒔𝒆𝒂𝒓𝒄𝒉 𝒚𝒐𝒖𝒓 𝒇𝒂𝒗𝒐𝒓𝒊𝒕𝒆 𝒎𝒐𝒗𝒊𝒆 𝒕𝒉𝒆𝒓𝒆 👇\n\n"
                    "<blockquote>"
...
async def refercall(bot, query):
    user_ref_points = await referdb.get_refer_points(query.from_user.id)
    btn = [[
        ...
        InlineKeyboardButton(
            f'⏳ {user_ref_points}', callback_data='ref_point'),
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    try:
        ...
        pass
    _, key = query.data.split("#")
    _fresh_val = FRESH.get(key)
    if not _fresh_val:
        return await query.answer("⚠️ Session expired. Please search again.", show_alert=True)
    search = _fresh_val.replace(" ", "_")
    req = query.from_user.id
    offset = 0
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    _fresh_val2 = FRESH.get(key)
    if not _fresh_val2:
        return await query.answer("⚠️ Session expired. Please search again.", show_alert=True)
    search = _fresh_val2.replace("_", " ")
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        await query.answer(url=f"https://t.me/{temp.U_NAME}?start=file_{query.message.chat.id}_{files.file_id}")
        ...
        btn2 = [[
            InlineKeyboardButton(
                "ᴄʜᴇᴄᴋ ᴍʏ ᴅᴍ 🗳️", url=f"https://t.me/{temp.U_NAME}")
        ]]
        ...
        try:
            ...
            # Answer immediately so spinner clears while we generate the link
            ...
            await log_msg.reply_text(
                text=f"•• ʟɪɴᴋ ɢᴇɴᴇʀᴀᴛᴇᴅ ғоʀ ɪᴅ #{user_id} \n•• ᵜʀᴇʀɴᴀᴍᴇ : {username} \n\n•• ᚤᛅᚫᛘ Nᚹᚸᛘ : {fileName}",
                quote=True,
                disable_web_page_preview=True,
            ...
            dreamcinezone = await query.edit_message_reply_markup(
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [
                        InlineKeyboardButton('📌 ᵊᴏɪɴ ᵜƣᴅᴀᴛᴇʀʜ ᴄʜᴀɴɴᴇʟ 📌', url=UPDATE_CHNL_LNK)
                    ]
                ])
            )
        ...
        await client.edit_message_media(
            query.message.chat.id,
            query.message.id,
            InputMediaPhoto('https://raw.githubusercontent.com/azizthekiller123/Auto-filter-bot-4/main/assets/qr_code.jpg')
        )
        await query.message.edit_text(
            text=script.DONATE_TXT.format(query.from_user.mention, QR_CODE, OWNER_UPI_ID),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        buttons = [[
            InlineKeyboardButton('Source Code 📜', url='https://t.me/Moviebot123'),
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ⇋', callback_data='about')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        _ref_pts = await referdb.get_refer_points(query.from_user.id)
        await query.answer(f'You Have: {_ref_pts} Refferal points.', show_alert=True)
...
async def advantage_spell_chok(client, message):
    ...
    if not movies:
        ...
        button = [
            ...
            [InlineKeyboardButton("📢 Backup Channel 1", url="https://t.me/backupchannek"),
             InlineKeyboardButton("📢 Backup Channel 2", url="https://t.me/BackupChannel5211")],
            [InlineKeyboardButton("📩 ʀᴇǫᴜᴇꜱᴛ ᴛᴏ Aᴅᴍɪɴ", callback_data=f"req_admin#{search[:50]}")]
        ]
    ...
    buttons.append([InlineKeyboardButton("📢 Backup Channel 1", url="https://t.me/backupchannek"),
                    InlineKeyboardButton("📢 Backup Channel 2", url="https://t.me/BackupChannel5211")])
    buttons.append([InlineKeyboardButton("📩 ʀᴇǫᴜᴇꜱᴛ ᴛᴏ Aᴅᴍɪɴ", callback_data=f"req_admin#{search[:50]}")])
...
async def request_to_admin(client, query):
    # Rate-limit: max 1 request per user per 60 s
    import time as _time
    _rl_now = _time.time()
    _rl_last = _req_cooldown.get(query.from_user.id, 0)
    if _rl_now - _rl_last < 60:
        return await query.answer("⏳ ᴘʟᴇᴀꜱᴇ ᴡᴀɪᴛ 1 ᴍɪɴᴜᴛᴇ ʙᴇꜰᴏʀᴇ ꜱᴇɴᴅɪɴɢ ᴀɴᴏᴛʜᴇʀ ʀᴇǫᴜᴇꜱᴛ.", show_alert=True)
    _req_cooldown[query.from_user.id] = _rl_now
    await query.answer("✅ ʏᴏᴜʀ ʀᴇǫᴜᴇꜱᴛ ʜᴀꜱ ʙᴇᴇɴ ꜱᴇɴᴛ ᴛᴏ ᴀᴅᴍɪɴꜱ!", show_alert=True)
    try:
        data = query.data.split("#", 1)
        movie_name = data[1] if len(data) > 1 else "Unknown"
        user = query.from_user
        # Prefer @username; fall back to first name
        requester = f"@{user.username}" if user.username else (user.first_name or str(user.id))
        text = f"#RequestFromYourGroup\n{movie_name}\n👤 {requester}"
        sent = False
        for admin_id in ADMINS:
            try:
                await client.send_message(chat_id=int(admin_id), text=text)
                sent = True
            except Exception:
                continue
        if not sent:
