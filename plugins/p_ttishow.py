...
from dreamxbotz.zzint import StartTime as botStartTime  # avoid circular import from bot.py
...
async def save_group(bot, message):
    ...
    if temp.ME in dreamx_check:
        ...
        try:
            await db.connect_group(message.chat.id, message.from_user.id if message.from_user else 0)
...
async def re_enable_chat(bot, message):
    ...
    # Bug fix: remove() raises ValueError after bot restart when in-memory list is empty
    try:
...
# /stats handled by plugins/stats.py
...
# NOTE: /ban and /unban are handled exclusively by plugins/admin_features.py
# Duplicate handlers were removed from here to prevent double-firing and double DB writes.
...
async def group_commands(client, message):
    # Bug fix: removed unused message.from_user.mention/.id access that crashed for anonymous admins
