...
async def ping_handler(request):
    return web.json_response({"status": "ok", "bot": "AutofilterADVbot"})
...
@routes.get(r"/static/{filename}", allow_head=True)
async def static_handler(request: web.Request):
    filename = request.match_info["filename"]
    # Strict path-traversal prevention: resolve absolute path and verify it stays inside static dir
    static_base = os.path.realpath('dreamxbotz/static')
    file_path = os.path.realpath(os.path.join(static_base, filename))
    # Must be strictly inside static_base (not the dir itself, not a symlink escape)
    if not file_path.startswith(static_base + os.sep):
